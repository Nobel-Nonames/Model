...
class TFDetector:
    ...
    def generate_detections_one_image(self, image, image_id, detection_threshold, label_map={}):
        ...
        try:
            ...
            for b, s, c in zip(boxes, scores, classes):
                if s > detection_threshold:
                    clss = str(int(c))
                    if clss != "3": ## not Vehicle
                        label = label_map[clss] if clss in label_map else clss
                        score = truncate_float(float(s),  # cast to float for json serialization
                                                   precision=CONF_DIGITS)
                        detection_entry = {
                            "best_class": clss,  # use string type for the numerical class label, not int
                            "best_probability": score,
                            "name": f'{label} {round(100 * score)}%',
                            "bbox": TFDetector.__convert_coords(b)
                        }
                        detections_cur_image.append(detection_entry)

            result["prediction"] = detections_cur_image
